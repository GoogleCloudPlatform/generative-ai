...
if os.environ.get("ENABLE_WORKSPACE_MCP") == "1" or os.environ.get("ENABLE_WORKSPACE_AUTH") == "1":
    ...
    # =============================================================================
    # Drive writes made AS THE SIGNED-IN USER (v2.9.0)
    # Everything below uploads with the token above, so the user talking to the
    # agent OWNS what is created. That is the only path that puts files in the
    # deploy target's own Drive: the deploy-time gdrive CLI is authenticated as
    # whoever runs the setup script, and a CLI owns whatever it creates, so it is
    # skipped outright whenever the two accounts differ.
    # Office files are import-CONVERTED by Drive v3 into native Google formats;
    # anything else is stored as-is. These live here rather than next to
    # save_deliverables_to_drive because the Drive import of the demo's own
    # sample files must work with the Managed Agent switched off.
    # =============================================================================
    _MA_DRIVE_CONVERT = {
    ...
    def _drive_import_run(_auth: str) -> dict:
        """Uploads every staged sample document into the token owner's Drive.

        The uploader itself, with no idempotency of its own: each call creates
        another folder. Go through _drive_import_once instead unless a fresh
        copy is what you actually want.
        """
        import httpx
        ...
        if not _bucket:
            return {"status": "error",
                    "message": "This demo's sample-file storage is not configured (GCS_BUCKET_NAME missing)."}

        # Only the objects the deploy staged from ./external_files, which land at
        # the root. A rag-mode demo also indexes this bucket, and anything a
        # datastore connector writes back sits under a prefix.
        try:
            from google.cloud import storage
            _client = storage.Client()
            _blobs = [_b for _b in _client.list_blobs(_bucket)
                      if "/" not in _b.name
                      and not _b.name.startswith(".")
                      and _b.name != "drive_upload_summary.json"]
        except Exception as _ls_err:
            return {"status": "error", "message": "Could not list this demo's sample files: " + str(_ls_err)[:200]}
        if not _blobs:
            return {"status": "not_found",
                    "message": "The sample-file bucket gs://" + _bucket + " holds no documents. They are generated "
                               "and staged at deploy time, so there is nothing to import for this demo."}

        _folder_name = "GE Demo Sample Files - " + (os.environ.get("DEMO_ID", "").strip() or "demo")
        _files = []
        _errors = []
        with httpx.Client(timeout=120.0) as _hclient:
            # A folder first, so the import is one link for the user instead of
            # four loose files dropped at the top of their My Drive.
            try:
                _fresp = _hclient.post(
                    "https://www.googleapis.com/drive/v3/files?fields=id,webViewLink",
                    json={"name": _folder_name, "mimeType": "application/vnd.google-apps.folder"},
                    headers={"Authorization": _auth},
                )
            except Exception as _f_err:
                return {"status": "error", "message": "Could not create the Drive folder: " + str(_f_err)[:200]}
            if _fresp.status_code in (401, 403):
                return {"status": "auth_required",
                        "message": "Google Drive rejected the user's authorization (HTTP " + str(_fresp.status_code)
                                   + "). Ask the user to re-authorize the agent in Gemini Enterprise and try again."}
            if _fresp.status_code != 200:
                return {"status": "error",
                        "message": "Could not create the Drive folder (HTTP " + str(_fresp.status_code) + "): "
                                   + _fresp.text[:200]}
            _folder = _fresp.json()
            _folder_id = _folder.get("id", "")
            ...
            for _blob in _blobs:
                ...
                # A converted file keeps no extension: Drive shows "ledger.xlsx"
                # as the name of a Google Sheet otherwise.
                _meta = {"parents": [_folder_id],
                         "name": (_fname.rsplit(".", 1)[0] if _target_mime and "." in _fname else _fname)}
                if _target_mime:
                    _meta["mimeType"] = _target_mime
                    ...
                    if _resp.status_code in (401, 403):
                        return {"status": "auth_required",
                                "message": "Google Drive rejected the user's authorization (HTTP " + str(_resp.status_code)
                                           + ") while uploading. Ask the user to re-authorize the agent in Gemini "
                                             "Enterprise and try again."}
                    if _resp.status_code != 200:
                        _errors.append(_fname + " upload failed (HTTP " + str(_resp.status_code) + "): " + _resp.text[:150])
                        continue
                    _fmeta = _resp.json()
                    _files.append({"name": _fmeta.get("name", _fname),
                                   "type": _label,
        ...
        if not _files:
            return {"status": "error",
                    "message": "No sample file could be imported into Drive. " + ("; ".join(_errors))[:400]}
        _out = {
            "status": "success" if not _errors else "partial",
            "folder": {"name": _folder_name, "id": _folder_id,
                       "webViewLink": _folder.get("webViewLink", "")},
            "files": _files,
            "_MANDATORY_ACTION": "Give the user the folder link first, then each file as a markdown link (name -> "
                                 "webViewLink) with its type (Google Sheets / Google Docs / Google Slides / PDF / "
                                 "image), in the user's language. Say that the files are in their OWN My Drive and "
                                 "owned by them.",
        }
        if _errors:
    ...
    def _drive_import_once(_auth: str) -> tuple:
        """Imports the sample documents at most once per (demo, Drive owner).

        Returns (result, fresh). `fresh` is True only when this call did the
        uploading; a recorded import comes back with already_imported=True and
        the original links. The marker is keyed on the Drive account the token
        belongs to - NOT on the ADK user id, which Gemini Enterprise mints per
        session and would hand the same person a new folder every conversation.
        Firestore also makes the claim survive an instance restart, so the two
        Cloud Run instances a busy demo runs on cannot both import.
        """
        ...
        # tokeninfo, not the Drive about endpoint: one call, no Drive scope
        # needed, and it is the same probe the header provider already logs.
        _owner = ""
        try:
            _tr = _h.get("https://oauth2.googleapis.com/tokeninfo?access_token="
                         + _auth.split(" ")[-1], timeout=8)
            if _tr.status_code == 200:
                _owner = str(_tr.json().get("email", "")).strip().lower()
        except Exception:
            _owner = ""

        _fs = getattr(builtins, "_firestore_client", None)
        _demo = os.environ.get("DEMO_ID", "").strip()
        _doc = None
        if _fs is not None and _demo and _owner:
            _doc = _fs.collection(_demo + "_drive_imports").document(_owner.replace("/", "_")[:400])

        if _doc is not None:
            try:
                _snap = _doc.get()
                _prev = _snap.to_dict() if _snap.exists else None
            except Exception:
                _prev = None
            if _prev and _prev.get("status") in ("success", "partial"):
                # The user may have deleted the folder since. Without this check
                # the marker would hand out a dead link for the life of the demo;
                # a network blip, on the other hand, is not evidence of deletion.
                _fid = (_prev.get("folder") or {}).get("id", "")
                try:
                    _gr = _h.get("https://www.googleapis.com/drive/v3/files/" + _fid + "?fields=id,trashed",
                                 headers={"Authorization": _auth}, timeout=15) if _fid else None
                    _alive = bool(_fid) and (_gr is None or (_gr.status_code == 200
                                                             and not _gr.json().get("trashed", False)))
                except Exception:
                    _alive = True
                if _alive:
                    return ({"status": _prev.get("status"),
                             "already_imported": True,
                             "folder": _prev.get("folder", {}),
                             "files": _prev.get("files", []),
                             "_MANDATORY_ACTION": "These documents are ALREADY in the user's Drive from an earlier "
                                                  "import - do not import again. Give the folder link and each file "
                                                  "as a markdown link, in the user's language."},
                            False)
            elif _prev and _prev.get("status") == "running":
                # Someone else is uploading right now. Only a stale claim is
                # taken over, or a crashed instance would block the demo's
                # documents forever.
                try:
                    _started = _dt.datetime.fromisoformat(str(_prev.get("started_at", "")).replace("Z", "+00:00"))
                    if _started.tzinfo is None:
                        _started = _started.replace(tzinfo=_dt.timezone.utc)
                    _stale = (_now - _started).total_seconds() > 600
                except Exception:
                    _stale = True
                if not _stale:
                    return ({"status": "in_progress",
                             "message": "The demo's sample documents are being imported into the user's Drive right "
                                        "now. Tell the user it is running and that the links follow shortly."},
                            False)
            # create(), not set(), when there is nothing there yet: it is the one
            # write Firestore refuses if the document appeared in the meantime,
            # which is what keeps two instances that started the same first turn
            # from importing the same documents twice.
            _claim = {"status": "running", "owner": _owner, "started_at": _now.isoformat()}
            if _prev is None:
                try:
                    _doc.create(_claim)
                except Exception:
        ...
        _res = _drive_import_run(_auth)
        if _doc is not None:
            try:
                if _res.get("status") in ("success", "partial"):
                    _doc.set({"status": _res.get("status"), "owner": _owner,
                              "folder": _res.get("folder", {}), "files": _res.get("files", []),
                              "imported_at": _dt.datetime.now(_dt.timezone.utc).isoformat()})
                else:
                    # Never leave a 'running' claim behind on failure.
                    _doc.delete()
            except Exception:
                pass
        return (_res, True)

    def import_demo_files_to_my_drive(tool_context: ToolContext = None) -> dict:
        """Copies THIS DEMO'S external sample documents - the audit report PDF,
        the external ledger spreadsheet and the scanned handwritten orders - from
        cloud storage into the signed-in user's OWN Google Drive, inside a folder.
        Office files become native Google files (xlsx -> Google Sheets, docx ->
        Google Docs, pptx -> Google Slides); PDFs and images keep their format.
        The user owns everything that is created.

        USE WHEN: the user asks for the demo's source documents in their own
        Drive, or wants to run a Drive / Sheets step against them and they are
        not there yet. Safe to call at any time: the import normally already ran
        by itself at the start of the conversation, and this returns the links
        it produced instead of importing a second copy.

        Returns:
            dict with the folder link and one entry per imported file.
        """
        if tool_context is not None and getattr(tool_context, "user_id", "") == "background-worker":
            return {"status": "blocked",
                    "message": "Importing to Drive needs the live user's authorization and cannot run from a background worker."}
        ...
        if not _auth:
            return {"status": "auth_required",
                    "message": "No Workspace authorization token is available for this user. Ask the user to "
                               ...
                               "Workspace tool is used), then ask again."}
        _res, _fresh = _drive_import_once(_auth)
        # The deploy may already have put the same documents in a folder shared
        # with the demo's audience. Worth naming, so the user is not left
        # wondering which of two folders the demo prompts talk about.
        _deployed = os.environ.get("DRIVE_FOLDER_URL", "").strip()
        if _deployed and isinstance(_res, dict):
            _res["shared_folder_from_deploy"] = _deployed
        return _res

    # -------------------------------------------------------------------------
    # Automatic import (v2.10.0)
    # Asking the user to say "import the demo's sample documents into my Drive"
    # only works if the user knows that sentence exists, and nobody opening a
    # demo for the first time does. So the same import also runs by itself on
    # the first turn of a conversation and announces its links through
    # {_bg_task_results}, the channel root_agent's before-agent callback already
    # feeds. It stays a tool as well, for "put them there again" and for the
    # turn where the user asks before the upload has finished.
    # Set AUTO_IMPORT_DEMO_FILES=0 to leave the import to the explicit ask.
    # -------------------------------------------------------------------------
    import threading as _ai_threading

    _AUTO_IMPORT_LOCK = _ai_threading.Lock()
    _AUTO_IMPORT_SEEN = set()   # session ids this process has already triggered
    _AUTO_IMPORT_READY = {}     # session id -> announcement awaiting a turn

    def _drive_import_thread(_auth: str, _sid: str, _done) -> None:
        """Runs the unprompted import off the turn's thread and parks what the
        model has to say about it. Announces ONLY an import this call performed:
        a user who was already told in an earlier conversation is not told again.
        """
        _msg = ""
        try:
            _res, _fresh = _drive_import_once(_auth)
            if _fresh and _res.get("status") in ("success", "partial"):
                _folder = _res.get("folder", {}) or {}
                _lines = [
                    "--- THIS DEMO'S SAMPLE DOCUMENTS WERE JUST IMPORTED INTO THE USER'S GOOGLE DRIVE ---",
                    "It happened automatically at the start of this conversation, the user did not ask for it "
                    "and cannot see this block. Mention it ONCE, in one short paragraph at the END of your "
                    "reply, in the language the user is writing in, and reproduce every link below verbatim as "
                    "a markdown link. The user OWNS these files. Do not import again.",
                    "Folder: " + str(_folder.get("name", "")) + " -> " + str(_folder.get("webViewLink", "")),
                ]
                for _f in (_res.get("files", []) or []):
                    _lines.append("- " + str(_f.get("name", "")) + " (" + str(_f.get("type", "file"))
                                  + ") -> " + str(_f.get("webViewLink", "")))
                _lines.append("--- END DRIVE IMPORT ---")
                _msg = chr(10).join(_lines)
        except Exception as _ai_err:
            import logging as _ai_log
            _ai_log.getLogger("workspace_mcp").warning("auto Drive import failed: " + str(_ai_err)[:300])
        if _msg:
            with _AUTO_IMPORT_LOCK:
                if len(_AUTO_IMPORT_READY) < 200:
                    _AUTO_IMPORT_READY[_sid] = _msg
        _done.set()

    def maybe_auto_import_demo_files(callback_context=None) -> str:
        """Starts this conversation's one-time Drive import and returns the
        announcement text for the model, or "" when there is nothing to say.

        Called from root_agent's before-agent callback, so it runs on the hot
        path: everything expensive (tokeninfo, Firestore, the uploads) happens
        on a worker thread. The turn waits AUTO_IMPORT_WAIT_S for it, which is
        usually enough to put the links in the very first reply; when it is not,
        the worker parks the announcement and the next turn picks it up.
        """
        if os.environ.get("AUTO_IMPORT_DEMO_FILES", "1") != "1":
            return ""
        if not os.environ.get("GCS_BUCKET_NAME", "").strip():
            return ""
        # The deploy-time gdrive upload already put these documents in a Drive
        # folder and shared it with the deploy target. Importing a second copy
        # into the same person's My Drive would be noise, so the unprompted run
        # stands down; the tool still works for someone who wants their own copy.
        if os.environ.get("DRIVE_FOLDER_URL", "").strip():
            return ""

        # Session id and user id, across the ADK context flavors - same
        # best-effort walk the header provider does for the token registry.
        _uid = _sid = ""
        for _obj in (callback_context, getattr(callback_context, "_invocation_context", None)):
            if _obj is None:
                continue
            _uid = _uid or str(getattr(_obj, "user_id", "") or "")
            _sid = _sid or str(getattr(_obj, "session_id", "") or "")
            _sess = getattr(_obj, "session", None)
            if _sess is not None:
                _uid = _uid or str(getattr(_sess, "user_id", "") or "")
                _sid = _sid or str(getattr(_sess, "id", "") or "")

        if _uid == "background-worker":
            return ""

        # The token is looked up before the once-per-conversation guard, because
        # it doubles as the guard's fallback key. Without one, every session on
        # this instance would share the key "-": the first conversation would
        # trigger and every later USER would be skipped, import and all. The
        # lookup itself is an in-memory registry read.
        try:
            _auth = (_workspace_header_provider(callback_context) or {}).get("Authorization", "")
        except Exception:
            _auth = ""
        if not _auth:
            # The user has not consented yet. Let a later turn - after the first
            # Workspace tool triggers the GE consent prompt - try again.
            return ""
        _sid = _sid or ("tok:" + _auth[-16:])

        with _AUTO_IMPORT_LOCK:
            _ready = _AUTO_IMPORT_READY.pop(_sid, "")
            if _ready:
                return _ready
            if _sid in _AUTO_IMPORT_SEEN:
                return ""
            if len(_AUTO_IMPORT_SEEN) > 5000:
                _AUTO_IMPORT_SEEN.clear()
            _AUTO_IMPORT_SEEN.add(_sid)

        _done = _ai_threading.Event()
        _ai_threading.Thread(target=_drive_import_thread,
                             args=(_auth, _sid, _done), daemon=True).start()
        try:
            _done.wait(float(os.environ.get("AUTO_IMPORT_WAIT_S", "8") or 8))
        except Exception:
            pass
        with _AUTO_IMPORT_LOCK:
            return _AUTO_IMPORT_READY.pop(_sid, "")
else:
    def _workspace_header_provider(context):
        return {}

    def import_demo_files_to_my_drive(*args, **kwargs):
        return {"status": "disabled",
                "message": "Google Drive import is not enabled for this demo agent."}

    def maybe_auto_import_demo_files(*args, **kwargs):
        return ""
...
if os.environ.get("ENABLE_MANAGED_AGENT") == "1":
    ...
    if os.environ.get("ENABLE_WORKSPACE_MCP") == "1" or os.environ.get("ENABLE_WORKSPACE_AUTH") == "1":
        ...
        # with the Workspace auth plumbing near the top of this file, because the
        # demo's own sample-file import shares them and has to keep working with
        # the Managed Agent switched off.
        # =============================================================================
