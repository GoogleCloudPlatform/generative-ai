...
# Per-demo values the deploy step passes in the environment. The generator's
# own copy of this viewer bakes them in at generation time; here the template
# ships verbatim, so the placeholders are filled once at startup.
_PLACEHOLDERS = (
    ("__GE_DASH_TITLE__", "DASHBOARD_TITLE", "Operations Console"),
    ("__GE_DASH_DESC__", "SYSTEM_DESCRIPTION", "Real-Time Operational Intelligence Dashboard"),
)

def _load_template(_name):
    # Served as-is (no Jinja): the pages' inline JavaScript is full of brace
    # sequences that Jinja would try to interpret.
    with open(os.path.join(_TEMPLATES_DIR, _name), encoding="utf-8") as _f:
        _page = _f.read()
    for _ph, _env, _default in _PLACEHOLDERS:
        # Escaped: these land in the page body, and the deploy step builds them
        # from the domain slug rather than from anything we control.
        _page = _page.replace(_ph, _html_mod.escape(os.environ.get(_env) or _default))
    return "\n" + _page
