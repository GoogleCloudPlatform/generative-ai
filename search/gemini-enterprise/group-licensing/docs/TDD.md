...
* **API Rate Limiting & Concurrency:**
    ...
    *   **Invalid Member Key (GC only):** If `members.hasMember` returns HTTP 400 (`ErrInvalidMemberKey`) for a licensed user — indicating the stored email is malformed or otherwise unresolvable by the Admin API — the user is skipped with a `WARN` log entry. No revocation is issued and the job continues. This guards against mistyped emails entered via the Discovery Engine UI, which lacks input validation.
* **Structured Logging:**
