...
def extract_query_entities(query: str, cities: List[str], pattern_type: str, output_type: str, debug: bool) -> dict:
    ...
    if unsupported_cities:
        raise ValueError(f"Following cities are not supported yet: {', '.join(unsupported_cities)}. Currently supporting only: {', '.join(STATE_MAPPING.keys())}")
...
class QueryAnalysisAgent:
    ...
    def _extract_entities(self, query: str) -> QueryEntities:
        ...
        try:
                ...
                contents=[
                    {
                        ...
                        Now extract entities from this query: """ + query
                    }
                ],
            ...
            if self.debug:
              print_debug(f"Raw extracted entities: {json.dumps(extracted, indent=2)}")
            ...
            raise ValueError(f"Could not extract required entities from query. Details: {str(e)}")

    def analyze(self, query: str) -> dict:
        if not self.debug: 
          rich_print(f"[bold yellow]ℹ️ Warning: Deciphering your cryptic commands!  It's like translating ancient hieroglyphs, but with more emojis.  Curious about my interpretations? debug=True reveals all! (And if you want to see the output of each agent stage, set stage_output=True!) 🤔📜 [/bold yellow]")
        ...
        try:
            ...
            if not entities.cities:
                raise ValueError("No cities mentioned in query. Please specify the city.")
            
            if entities.pattern_type == PatternType.COMPARISON and len(entities.cities) < 2:
                raise ValueError("Comparison requires at least two cities. Please mention both cities.")
