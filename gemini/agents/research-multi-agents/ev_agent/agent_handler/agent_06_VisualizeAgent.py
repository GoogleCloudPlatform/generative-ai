...
def create_comprehensive_city_analysis(data):
    ...
    charging_data = pd.DataFrame([
        {'type': 'DC Fast', 'count': city_data.ev_data.charging_capabilities.by_type['dc_fast'].count},
        {'type': 'Level 2', 'count': city_data.ev_data.charging_capabilities.by_type['level2'].count},
        {'type': 'Level 1', 'count': city_data.ev_data.charging_capabilities.by_type['level1'].count}
    ])
    
    charging_fig = px.bar(charging_data, x='type', y='count', 
    ...
    connector_fig = px.pie(connector_data, values='count', names='type',
    ...
    network_fig = px.bar(network_data, x='network', y='count',
                        title=f'Network Distribution - {city_name}')
    figures['network_distribution'] = network_fig
    
    # Access & Payment Methods
    access_data = pd.DataFrame([
        {'method': 'Credit Card', 'percentage': city_data.ev_data.accessibility.payment_methods['credit_card']['percentage']},
        {'method': 'Mobile Pay', 'percentage': city_data.ev_data.accessibility.payment_methods['mobile_pay']['percentage']},
        {'method': 'Network Card', 'percentage': city_data.ev_data.accessibility.payment_methods['network_card']['percentage']},
        {'method': '24/7 Access', 'percentage': city_data.ev_data.accessibility.access_type['24_7_access']['percentage']}
    ])
    access_fig = px.bar(access_data, x='method', y='percentage',
    ...
    transport_data = pd.DataFrame([
        ...
        {'type': 'Train Stations', 'count': city_data.summary.transport.train_stations},
    ...
    transport_fig.add_trace(
        go.Bar(x=transport_data['type'], y=transport_data['count']),
        row=1, col=1
    )
    ...
    transport_fig.add_trace(
        go.Bar(x=road_data['type'], y=road_data['count']),
        row=1, col=2
    )
    
    # Parking Facilities
    parking_data = pd.DataFrame([
        {'type': 'Surface Parking', 'count': city_data.summary.parking.surface_parking},
        {'type': 'Parking Structures', 'count': city_data.summary.parking.parking_structures},
        {'type': 'Street Parking', 'count': city_data.summary.parking.street_parking},
        {'type': 'EV Charging', 'count': city_data.summary.parking.ev_charging}
    ])
    transport_fig.add_trace(
        go.Bar(x=parking_data['type'], y=parking_data['count']),
        row=2, col=1
    )
    
    # EV vs Traditional Infrastructure
    infra_comparison = pd.DataFrame([
        {'type': 'EV Charging Stations', 'count': city_data.summary.automotive.ev_charging_stations},
        {'type': 'Fuel Stations', 'count': city_data.summary.automotive.fuel_stations},
        {'type': 'Car Dealerships', 'count': city_data.summary.automotive.car_dealerships},
        {'type': 'Car Repair', 'count': city_data.summary.automotive.car_repair}
    ])
    transport_fig.add_trace(
        go.Bar(x=infra_comparison['type'], y=infra_comparison['count']),
        row=2, col=2
    )
    
    transport_fig.update_layout(height=800, title_text=f"Transportation Infrastructure - {city_name}")
    ...
    retail_data = pd.DataFrame([
        {'type': 'Shopping Centers', 'count': city_data.summary.retail.shopping_centres},
        {'type': 'Supermarkets', 'count': city_data.summary.retail.supermarkets},
        {'type': 'Department Stores', 'count': city_data.summary.retail.department_stores},
        {'type': 'Convenience Stores', 'count': city_data.summary.retail.convenience_stores}
    ])
    amenities_fig.add_trace(
        go.Bar(x=retail_data['type'], y=retail_data['count']),
        row=1, col=1
    )
    ...
    amenities_fig.add_trace(
        go.Bar(x=food_ent_data['type'], y=food_ent_data['count']),
        row=1, col=2
    )
    
    # Emergency Services
    emergency_data = pd.DataFrame([
        {'type': 'Police Stations', 'count': city_data.summary.emergency.police_stations},
        {'type': 'Fire Stations', 'count': city_data.summary.emergency.fire_stations},
        {'type': 'Hospitals', 'count': city_data.summary.healthcare.hospitals},
        {'type': 'Clinics', 'count': city_data.summary.healthcare.clinics}
    ])
    amenities_fig.add_trace(
        go.Bar(x=emergency_data['type'], y=emergency_data['count']),
        row=2, col=1
    )
    ...
    amenities_fig.add_trace(
        go.Bar(x=public_data['type'], y=public_data['count']),
        row=2, col=2
    )
    
    amenities_fig.update_layout(height=800, title_text=f"Urban Amenities and Services - {city_name}")
    figures['amenities'] = amenities_fig
    
    # 4. Area Analysis (Pie Chart)
    area_data = pd.DataFrame([
        {'type': 'Total Area', 'area': city_data.summary.area_metrics.total_area_sqkm},
        {'type': 'Water Area', 'area': city_data.summary.area_metrics.water_area_sqkm},
        {'type': 'Green Area', 'area': city_data.summary.area_metrics.green_area_sqkm},
        {'type': 'Built Area', 'area': city_data.summary.area_metrics.built_area_sqkm}
    ])
    
    area_fig = px.pie(area_data, values='area', names='type',
    ...
    if len(data.cities_data) > 1:
        ...
        for city in data.cities_data:
            cities_data.append({
                ...
                'dc_fast_count': city.ev_data.charging_capabilities.by_type['dc_fast'].count,
                'level2_count': city.ev_data.charging_capabilities.by_type['level2'].count,
                'level1_count': city.ev_data.charging_capabilities.by_type['level1'].count,
        ...
        # 4. Urban Amenities
        amenity_metrics = ['shopping_centres', 'supermarkets', 'restaurants', 'cafes', 
