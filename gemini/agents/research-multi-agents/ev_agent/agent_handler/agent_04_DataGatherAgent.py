...
class ColorPrinter:
    ...
    def print_message(message: str, msg_type: str = 'info', bold: bool = False):
        ...
        ColorPrinter.print_html(
            prefix + message,
            ColorPrinter.COLORS[msg_type],
            bold
        )
...
class DataGatherAgent:
    ...
    async def _gather_city_data(self, city: str, state: str) -> CityData:
        ...
        if self.debug: 
          self._print_monologue(f"Investigating {city}, {state}... *beep boop* Time to multitask!")
        ...
        try:
            ...
            if self.debug: 
              self._print_monologue(f"Successfully gathered all data for {city}! *happy robot noises*")
    ...
    async def _get_city_summary(self, city: str, state: str) -> Dict:
        ...
        try:
            summary = await asyncio.to_thread(
                self.processor.create_city_summary,
                payload
            )
            
            if self.debug:
              self.printer.print_message(
                f"Received city summary for {city}",
                'success'
            )
            return summary
            ...
            if self.debug:
              self.printer.print_message(
                f"Error in city summary for {city}: {str(e)}",
                'error'
            )
            raise
    ...
    async def _get_ev_data(self, city: str, state: str) -> Dict:
        ...
        try:
            ...
            if self.debug:
              self.printer.print_message(
                f"Error in EV data for {city}: {str(e)}",
                'error'
            )
            raise
    ...
    async def process(self, input_data: Dict) -> DataGatherAgentOutput:
        ...
        if self.debug:
          self._print_monologue("Initializing data gathering mission... *whirring sounds*")
        ...
        try:
            ...
            cities_data = [
                city if isinstance(city, CityData) else
                CityData(
