...
from smolagents import CodeAgent, HfApiModel, Tool, ActionStep
...
class DatasetGenerator(weave.Model):
            ...
            if isinstance(log, ActionStep) and log.tool_calls:
                ...
                try:
                    ...
                    agent_step = AgentStep(
                        ...
                        llm_response=log.model_output_message.content if log.model_output_message else ""
                    )
    ...
    @weave.op()
    def _judge_final_response(self, prompt: str, response: str, tools_used: List[str]) -> Tuple[float, str]:
        ...
        if self.debug:
            self.console.print(f"[bold cyan]Final Response Score:[/bold cyan] {result.score:.2f}")
            self.console.print(f"[bold cyan]Reasoning:[/bold cyan] {result.reasoning}\n")
    ...
    def _judge_step(self, prompt: str, step: AgentStep) -> Tuple[float, str]:
        ...
        if step.tool_name == "python_interpreter":
            ...
            tool_execution_info = f"Tool: {step.tool_name}\nArguments: {step.tool_args}\n"
    ...
    @weave.op()
    def _judge_trajectory(self, prompt: str, steps: List[AgentStep]) -> Tuple[float, str]:
        ...
        if self.debug:
            self.console.print(f"[bold cyan]Trajectory Score:[/bold cyan] {result.score:.2f}")
            self.console.print(f"[bold cyan]Reasoning:[/bold cyan] {result.reasoning}\n")
    ...
    def _extract_tool_names_from_step(self, step: AgentStep) -> List[str]:
        ...
        if step.tool_name == "python_interpreter":
            ...
            if self.debug:
                ...
                if tools_found or any(tool in code for tool in ['def ', 'class ', 'import ']):
                    self.console.print(Panel(
    ...
    def generate_dataset(self, input_prompts: List[str]) -> List[EvaluationExample]:
        ...
        for i, prompt in enumerate(input_prompts, 1):
            if self.debug:
                self.console.rule(f"[bold red]Processing Prompt {i}/{len(input_prompts)}")
            ...
            step_scores_with_reasoning = [
                self._judge_step(prompt, step)
                for step in trajectory.steps
            ]
            
            trajectory_score, trajectory_reasoning = self._judge_trajectory(prompt, trajectory.steps)
            
            # Apply thresholds
            if final_response_score < self.thresholds["final_response"]:
                if self.debug:
                    self.console.print(f"[red]Final response score {final_response_score:.2f} below threshold {self.thresholds['final_response']}")
                continue
            
            if any(score < self.thresholds["single_step"] for score, _ in step_scores_with_reasoning):
                if self.debug:
                    self.console.print(f"[red]Some step scores below threshold {self.thresholds['single_step']}")
                continue
            
            if trajectory_score < self.thresholds["trajectory"]:
                if self.debug:
                    self.console.print(f"[red]Trajectory score {trajectory_score:.2f} below threshold {self.thresholds['trajectory']}")
                continue
