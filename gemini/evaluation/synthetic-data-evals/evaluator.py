...
class AgentEvaluator:
    ...
    def _render_summary_table(self, summary_metrics: Dict, include_ratings: bool = True) -> Table:
        """
        ...
        if "Row Count" in summary_metrics:
            if include_ratings:
                table.add_row(
                    "Row Count", 
                    f"{summary_metrics['Row Count']:.2f}", 
                    ""
                )
            else:
                table.add_row(
    ...
    def _render_detailed_metrics_table(self, summary_metrics: Dict) -> Table:
        ...
        for base_metric in sorted(base_metrics):
            ...
            if all(key in summary_metrics for key in [mean_key, std_key, min_key, max_key]):
                # Format the display name
                display_name = base_metric
                if '/score' in display_name:
                    display_name = display_name.replace('/score', '')
    ...
    def run_evaluation(self, agent, eval_dataset: pd.DataFrame, 
                       output_dir: str = "evaluation_results",
                       weave_project: str = "agent_evaluation") -> Dict:
        """
        ...
        for i, row in eval_dataset.iterrows():
            ...
            try:
                ...
                eval_dataset.at[i, 'tool_usage'] = json.dumps(tool_usage)  # Convert to JSON string
                ...
                if self.verbosity >= 1:
                    console.print(f"[bold red]Error running agent on example {i+1}:[/bold red] {str(e)}")
        ...
        if self.verbosity >= 1:
            self.console.print(f"[bold blue]Starting evaluation with run ID: [/bold blue][cyan]{run_id}[/cyan]")
        ...
        if hasattr(eval_result, 'metrics_table') and isinstance(eval_result.metrics_table, pd.DataFrame):
            ...
            if self.verbosity >= 3:
                self.console.print(f"[bold cyan]Metrics table columns:[/bold cyan] {list(eval_result.metrics_table.columns)}")
            ...
            for col in eval_result.metrics_table.columns:
                ...
                if ('score' in col.lower() or 
                    'accuracy' in col.lower() or 
                    'quality' in col.lower() or 
                    'match' in col.lower() or
                    'coherence' in col.lower()):
                    score_columns.append(col)
            
            if self.verbosity >= 2:
                self.console.print(f"[bold cyan]Found score columns:[/bold cyan] {score_columns}")
            
            # If no score columns found, try to use numeric columns as fallback
            if not score_columns:
                numeric_cols = eval_result.metrics_table.select_dtypes(include=['number']).columns.tolist()
                score_columns = [col for col in numeric_cols if col != 'id']
                if self.verbosity >= 1:
                    self.console.print(f"[yellow]No score columns found, using numeric columns:[/yellow] {score_columns}")
            ...
            for col in score_columns:
                try:
                    ...
                    if pd.api.types.is_numeric_dtype(eval_result.metrics_table[col]):
                        ...
                        if self.verbosity >= 2:
                            self.console.print(f"[green]✓[/green] Processed column {col}: mean={mean:.2f}, std={std:.2f}")
                    elif self.verbosity >= 2:
                        self.console.print(f"[yellow]Skipping non-numeric column: {col}[/yellow]")
                except Exception as e:
                    if self.verbosity >= 1:
            ...
            if len(summary_metrics) <= 1:  # Only Row Count
                if self.verbosity >= 1:
                    self.console.print("[yellow]Warning: No metrics found in evaluation results. Adding dummy metrics for testing.[/yellow]")
                
                # Extract any available metrics from the evaluation result object
                if hasattr(eval_result, 'metrics') and self.verbosity >= 2:
                    self.console.print(f"[cyan]Available metrics in eval_result.metrics: {eval_result.metrics}[/cyan]")
                    ...
                    if isinstance(eval_result.metrics, dict):
                        for metric_name, metric_value in eval_result.metrics.items():
                            if isinstance(metric_value, (int, float)):
                                summary_metrics[f"{metric_name}/Mean"] = float(metric_value)
                                summary_metrics[f"{metric_name}/Std"] = 0.0
                                summary_metrics[f"{metric_name}/Min"] = float(metric_value)
                                summary_metrics[f"{metric_name}/Max"] = float(metric_value)
                
                # If still no metrics, add dummy metrics
                if len(summary_metrics) <= 1:
                    for metric in ["tool_selection_accuracy", "reasoning_quality", "response_correctness", "trajectory_match", "coherence"]:
            ...
            if self.verbosity >= 2:
                self.console.print(f"[cyan]Available attributes on eval_result: {dir(eval_result)}[/cyan]")
        ...
        if self.use_weave:
            ...
            if weave_project:
                try:
                    ...
                    if self.verbosity >= 1:
                        self.console.print(f"[yellow]Warning: Could not initialize Weave project: {str(e)}[/yellow]")
            
            # Create a lookup dictionary for agent responses
            response_lookup = {response["id"]: response for response in results["examples"]}
            ...
            if hasattr(eval_result, 'metrics_table') and isinstance(eval_result.metrics_table, pd.DataFrame):
                for _, row in eval_result.metrics_table.iterrows():
                    ...
                    if example_id:
                        ...
                        metrics = {}
                        for col in eval_result.metrics_table.columns:
                            if 'score' in col.lower() and isinstance(row[col], (int, float)):
            ...
            def pass_through_function(id: str, context: str):
                ...
                if matching_examples:
                    ...
                    try:
                        result["tool_usage"] = json.loads(example.get("tool_usage", "[]"))
                    except:
                        result["tool_usage"] = []
                    ...
                    try:
                        result["trajectory"] = json.loads(example.get("trajectory", "[]"))
                    except:
                        result["trajectory"] = []
                    ...
                    if memory and hasattr(memory, 'steps'):
                        for step in memory.steps:
                            ...
                            if hasattr(step, 'model_output_message') and step.model_output_message:
                                step_info["model_output"] = step.model_output_message.content
            ...
            def tool_selection_scorer(model_output: dict) -> dict:
                ...
                return {
                    ...
                    "tool_names": [t.get("tool_name", "") for t in tool_usage if isinstance(t, dict)] or tool_names_from_steps
                }
            ...
            def trajectory_analysis_scorer(model_output: dict) -> dict:
                ...
                return {
                    ...
                    "tools_in_trajectory": len([step for step in trajectory if isinstance(step, dict) and step.get("tool_name")]) or 
                                          len([step for step in steps if "tool_calls" in step and step["tool_calls"]])
                }
            ...
            try:
                ...
                # Store Weave run ID
                weave_run_id = weave_results.run_id if hasattr(weave_results, 'run_id') else None
                if self.verbosity >= 1 and weave_run_id:
                    self.console.print(f"[bold]View Weave results at:[/bold] [cyan]https://wandb.ai/{weave_project}/runs/{weave_run_id}[/cyan]")
            except Exception as e:
                if self.verbosity >= 1:
                    self.console.print(f"[red]Error running Weave evaluation: {str(e)}[/red]")
                weave_run_id = None
        else:
            weave_run_id = None
                ...
                if isinstance(log, ActionStep) and log.tool_calls:
                    for tool_call in log.tool_calls:
                        step_data = {
                            ...
                            "tool_output": log.action_output,
                            "reasoning": log.model_output_message.content if log.model_output_message else ""
                        }
    ...
    def _generate_visualizations(self, eval_result, output_dir):
        ...
        if not hasattr(eval_result, 'metrics_table') or not isinstance(eval_result.metrics_table, pd.DataFrame) or eval_result.metrics_table.empty:
            if self.verbosity >= 1:
        ...
        try:
            ...
            if self.verbosity >= 1:
                results_table = Table(title="[bold]Evaluation Results Summary[/bold]", 
            ...
            if self.verbosity >= 1:
                self.console.print(f"[red]Error generating summary statistics: {str(e)}[/red]")
                ...
                if len(metrics_df['difficulty'].unique()) > 0 and len(score_columns) > 0:
                    ...
                    if not pivot_df.empty:
                        ...
                        if self.verbosity >= 1:
                            ...
                            difficulty_table = Table(title="[bold]Scores by Difficulty Level[/bold]", 
                                                    show_header=True, header_style="bold cyan")
