...
import json
import pandas as pd
from vertex_model import VertexAIServerModel
import os
from dotenv import load_dotenv
import time
import weave
from vertex_model import WeaveVertexAIServerModel
import config
import numpy as np
    ...
    def forward(self, order_id: str) -> str:
        ...
        if not order.empty:
            ...
            if not product.empty:
                ...
                order_data['product_category'] = product_data.get('category', 'Unknown Category')
            ...
            if order_data.get('status') in ['processing', 'shipped']:
                if pd.notna(order_data.get('estimated_delivery_date')):
                    order_data['delivery_estimate'] = order_data.get('estimated_delivery_date')
...
class CategoryBrowseTool(Tool):
    ...
    def forward(self, category: str) -> str:
        ...
        if not matches.empty:
            ...
            for _, product in matches.sort_values('price', ascending=False).head(5).iterrows():
...
class PriceCheckTool(Tool):
    ...
    inputs = {
        "product_id": {
            "type": "string",
            "description": "Product ID to check price for"
        }
    }
    output_type = "string"
...
class CustomerOrderHistoryTool(Tool):
    ...
    def forward(self, customer_id: str) -> str:
        ...
        if not customer_orders.empty:
            ...
            for _, order in customer_orders.sort_values('order_date', ascending=False).head(5).iterrows():
                order_data = {
...
def load_realistic_datasets(products_path: str = "data/products.csv", orders_path: str = "data/orders.csv", overwrite: bool = True, language: str = "en"):
    ...
    if overwrite or not (os.path.exists(products_path) and os.path.exists(orders_path)):
        try:
            ...
            try:
                ...
                print("Make sure you have the Kaggle API credentials set up (~/.kaggle/kaggle.json)")
                raise
            ...
            if not csv_files:
                raise FileNotFoundError("Could not find train.csv in the extracted dataset")
            ...
            for product_id, group in reviews_df.groupby('product_id'):
                ...
                description = f"Quality {clean_category.lower()} product with ID {short_id}."
            ...
            for _, purchase in customer_data.iterrows():
                ...
                if status_choice < 0.7:  # 70% delivered
                    status = 'delivered'
                    delivery_date = (datetime.strptime(order_date, '%Y-%m-%d') + timedelta(days=random.randint(1, 7))).strftime('%Y-%m-%d')
                    ...
                    delivery_date = None
                    tracking_number = f"TRK{random.randint(10000000, 99999999)}"
                    estimated_delivery_date = (datetime.strptime(order_date, '%Y-%m-%d') + timedelta(days=random.randint(1, 7))).strftime('%Y-%m-%d')
                elif status_choice < 0.95:  # 10% processing
                    status = 'processing'
                    delivery_date = None
                    tracking_number = None
                    estimated_delivery_date = (datetime.strptime(order_date, '%Y-%m-%d') + timedelta(days=random.randint(3, 10))).strftime('%Y-%m-%d')
                else:  # 5% cancelled
                    status = 'cancelled'
                    delivery_date = None
                    tracking_number = None
            ...
            print(f"Created product ({len(products)} items) and order ({len(orders)} orders) datasets from Amazon Reviews Multi data")
