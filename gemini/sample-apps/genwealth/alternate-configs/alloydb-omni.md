...
1. Create a new connection to AlloyDB Omni in pgAdmin using the external IP of the `alloydb-svc` load balancer created above. Use the `postgres` user and the password you defined in the kubernetes manifest earlier when you created the AlloyDB Mmni cluster.
...
1. Create tables and indexes.
    ...
    CREATE TABLE IF NOT EXISTS conversation_history (
        ...
        user_id INTEGER, 
        user_prompt TEXT, 
    user_prompt_embedding VECTOR(768) GENERATED ALWAYS AS (embedding('textembedding-gecko@003', user_prompt)::vector) STORED,
        ai_response TEXT,
    ai_response_embedding VECTOR(768) GENERATED ALWAYS AS (embedding('textembedding-gecko@003', ai_response)::vector) STORED,
