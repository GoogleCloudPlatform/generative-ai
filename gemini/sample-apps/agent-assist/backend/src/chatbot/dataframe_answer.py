...
# SQL_PROMPT is a string template that is used to generate the SQL
# query prompt for the TextBison model.
# It takes a question as input and generates a SQL query that can
# be used to answer the question.
SQL_PROMPT = """
You are an SQL Expert. Given an input question, use sqlite syntax to \
generate a sql query by choosing
one or multiple of the following tables. Write query in between <SQL></SQL>.

The first table is a User Policy table whose name is df1 where each row \
represents the policy of a user.
...
policy_amount, platform.

The second table is a likes table whose name is df2 where each row \
represents a social media post.
The column keys are : campaignID, websiteVisitors, instaLikes, instaComments, \
facebookLikes, facebookComments, facebookShares, numberOfMailSent, \
numberOfMailOpen, facebookSales, newspaperSales, TVAdsSales, instaSales, \
mailSales, telephoneSales, totalSales.


Please provide the SQL query for this question. Note that the SQL query \
should be a valid query. It should not use columns which are not present \
in the table.
Question: {question}
Query:
"""

# FINAL_ANSWER_PROMPT is a string template that is used to generate the
# final answer prompt for the TextBison model.
# It takes a question and a dataframe as input and generates a natural
# language answer to the question.
FINAL_ANSWER_PROMPT = """
...
The output has been returned as a dataframe in pandas which would be \
