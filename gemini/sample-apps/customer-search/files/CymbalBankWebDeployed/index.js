...
app.get('/', async (req, res) => {
  if (!template) {
    try {
      template = handlebars.compile(readFileSync('public/index.html.hbs', 'utf8'));
    } catch (e) {
      console.error(e);
...
app.get("/search", async (req, res) => {
  if (!template_search) {
    try {
      template_search = handlebars.compile(readFileSync('public/search.html.hbs', 'utf8'));
    } catch (e) {
      console.error(e);
      res.status(500).send("Internal Server Error");
    }
...
app.get("/saving", async (req, res) => {
  if (!template_saving) {
    try {
      template_saving = handlebars.compile(readFileSync('public/saving.html.hbs', 'utf8'));
    } catch (e) {
      console.error(e);
      res.status(500).send("Internal Server Error");
    }
...
app.get("/credit_card", async (req, res) => {
  if (!template_credit_card) {
    try {
      template_credit_card = handlebars.compile(readFileSync('public/credit_card.html.hbs', 'utf8'));
    } catch (e) {
      console.error(e);
      res.status(500).send("Internal Server Error");
    }
...
app.get("/custom_credit_card", async (req, res) => {
  if (!template_custom_credit_card) {
    try {
      template_custom_credit_card = handlebars.compile(readFileSync('public/custom_credit_card.html.hbs', 'utf8'));
    } catch (e) {
      console.error(e);
      res.status(500).send("Internal Server Error");
    }
...
app.get("/neft", async (req, res) => {
  if (!template_neft) {
    try {
      template_neft = handlebars.compile(readFileSync('public/neft.html.hbs', 'utf8'));
    } catch (e) {
      console.error(e);
      res.status(500).send("Internal Server Error");
    }
...
app.get("/upi", async (req, res) => {
  if (!template_upi) {
    try {
      template_upi = handlebars.compile(readFileSync('public/upi.html.hbs', 'utf8'));
    } catch (e) {
      console.error(e);
...
app.get("/imps", async (req, res) => {
  if (!template_imps) {
    try {
      template_imps = handlebars.compile(readFileSync('public/imps.html.hbs', 'utf8'));
    } catch (e) {
      console.error(e);
      return res.status(500).send('Internal Server Error');
    }
...
app.get("/debit_card", async (req, res) => {
  if (!template_debit_card) {
    try {
      template_debit_card = handlebars.compile(readFileSync('public/debit_card.html.hbs', 'utf8'));
    } catch (e) {
      console.error(e);
      return res.status(500).send('Internal Server Error');
    }
...
app.get("/recharge", async (req, res) => {
  if (!template_recharge) {
    try {
      template_recharge = handlebars.compile(readFileSync('public/recharge.html.hbs', 'utf8'));
    } catch (e) {
      console.error(e);
      return res.status(500).send('Internal Server Error');
    }
...
app.get("/electricity", async (req, res) => {
  if (!template_electricity) {
    try {
      template_electricity = handlebars.compile(readFileSync('public/electricity.html.hbs', 'utf8'));
    } catch (e) {
      console.error(e);
      return res.status(500).send('Internal Server Error');
    }
...
app.get("/insurance_premium", async (req, res) => {
  if (!template_insurance_premium) {
    try {
      template_insurance_premium = handlebars.compile(readFileSync('public/insurance_premium.html.hbs', 'utf8'));
    } catch (e) {
      console.error(e);
      return res.status(500).send('Internal Server Error');
    }
...
app.get("/mutual_funds", async (req, res) => {
  if (!template_mutual_funds) {
    try {
      template_mutual_funds = handlebars.compile(readFileSync('public/mutual_funds.html.hbs', 'utf8'));
    } catch (e) {
      console.error(e);
      return res.status(500).send('Internal Server Error');
    }
...
app.get("/ipo", async (req, res) => {
  if (!template_ipo) {
    try {
      template_ipo = handlebars.compile(readFileSync('public/ipo.html.hbs', 'utf8'));
    } catch (e) {
      console.error(e);
      return res.status(500).send('Internal Server Error');
    }
...
app.get("/stocks", async (req, res) => {
  if (!template_stocks) {
    try {
      template_stocks = handlebars.compile(readFileSync('public/stocks.html.hbs', 'utf8'));
    } catch (e) {
      console.error(e);
      return res.status(500).send('Internal Server Error');
    }
...
app.get("/current", async (req, res) => {
  if (!template_current) {
    try {
      template_current = handlebars.compile(readFileSync('public/current.html.hbs', 'utf8'));
    } catch (e) {
      console.error(e);
      return res.status(500).send('Internal Server Error');
    }
...
app.get("/salary", async (req, res) => {
  if (!template_salary) {
    try {
      template_salary = handlebars.compile(readFileSync('public/salary.html.hbs', 'utf8'));
    } catch (e) {
      console.error(e);
      return res.status(500).send('Internal Server Error');
    }
...
app.get("/fixed_deposit", async (req, res) => {
  if (!template_fixed_deposit) {
    try {
      template_fixed_deposit = handlebars.compile(readFileSync('public/fixed_deposit.html.hbs', 'utf8'));
    } catch (e) {
      console.error(e);
      return res.status(500).send('Internal Server Error');
    }
...
app.get("/recurring_deposit", async (req, res) => {
  if (!template_recurring_deposit) {
    try {
      template_recurring_deposit = handlebars.compile(readFileSync('public/recurring_deposit.html.hbs', 'utf8'));
    } catch (e) {
      console.error(e);
      return res.status(500).send('Internal Server Error');
    }
...
app.get("/loans", async (req, res) => {
  if (!template_loans) {
    try {
      template_loans = handlebars.compile(readFileSync('public/loans.html.hbs', 'utf8'));
    } catch (e) {
      console.error(e);
      return res.status(500).send('Internal Server Error');
    }
...
app.get("/saving/terms_and_condition", async (req, res) => {
  if (!template_saving_terms) {
    try {
      template_saving_terms = handlebars.compile(readFileSync('public/saving_terms_and_condition.html.hbs', 'utf8'));
    } catch (e) {
      console.error(e);
      return res.status(500).send('Internal Server Error');
    }
...
app.get("/calculator", async (req, res) => {
  if (!template_calculator) {
    try {
      template_calculator = handlebars.compile(readFileSync('public/calculator.html.hbs', 'utf8'));
    } catch (e) {
      console.error(e);
      return res.status(500).send('Internal Server Error');
    }
...
app.get("/loans/terms_and_condition", async (req, res) => {
  if (!template_loan_tc) {
    try {
      template_loan_tc = handlebars.compile(readFileSync('public/loans_T&C.html.hbs', 'utf8'));
    } catch (e) {
      console.error(e);
      return res.status(500).send('Internal Server Error');
    }
...
app.get('/loans/agreement', async (req, res) => {
  if (!template_loan_agreement) {
    try {
      template_loan_agreement = handlebars.compile(readFileSync('public/loan_agreement.html.hbs', 'utf8'));
    } catch (e) {
      console.error(e);
      return res.status(500).send('Internal Server Error');
    }
