...
def create_cymbuddy_df():
  ...
  for i in range(len(json_data["flow"]["fulfillments"])):
    if json_data["flow"]["fulfillments"][i]["value"]["webhook"]["url"].split("/")[-1]+"_url" in file_dict:
      json_data["flow"]["fulfillments"][i]["value"]["webhook"]["url"] = file_dict[json_data["flow"]["fulfillments"][i]["value"]["webhook"]["url"].split("/")[-1]+"_url"]
    else:
      search_str = json_data["flow"]["fulfillments"][i]["value"]["webhook"]["url"].split("/")[-1]
      search_str = search_str.replace("_", "-")
      if search_str+"_url" in file_dict:
        json_data["flow"]["fulfillments"][i]["value"]["webhook"]["url"] = file_dict[search_str+"_url"]
      else:
        print(search_str)

  for i in range(len(json_data["referencedFlows"][0]["fulfillments"])):
    if json_data["referencedFlows"][0]["fulfillments"][i]["value"]["webhook"]["url"].split("/")[-1]+"_url" in file_dict:
      json_data["referencedFlows"][0]["fulfillments"][i]["value"]["webhook"]["url"] = file_dict[json_data["referencedFlows"][0]["fulfillments"][i]["value"]["webhook"]["url"].split("/")[-1]+"_url"]
    else:
      search_str = (
          json_data["referencedFlows"][0]["fulfillments"][i]["value"]["webhook"]["url"].split("/")[-1]
      )
      search_str = search_str.replace("_", "-")
      if search_str+"_url" in file_dict:
        json_data["referencedFlows"][0]["fulfillments"][i]["value"]["webhook"]["url"] = (
