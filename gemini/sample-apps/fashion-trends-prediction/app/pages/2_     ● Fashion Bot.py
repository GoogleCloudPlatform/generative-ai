...
country = st.selectbox(
    'Select Country',
    st.session_state['JSONdata']['finaldata'].keys()
)
...
if hist_key not in st.session_state:
    ...
    with st.spinner("Loading Chatbot..."):
        ...
        bot.send_message("answer based on data you have about " +
                         category + "", generation_config=generation_config, safety_settings=safety_settings)
        bot.send_message(
            "based on frequency of semantically similar items, what are top 5 frequent items?", generation_config=generation_config, safety_settings=safety_settings)
        time.sleep(10)
        bot.send_message("don't mention counts with answer ",
                         generation_config=generation_config, safety_settings=safety_settings)
        bot.send_message(
            "assume popularity depends on frequency in given data", generation_config=generation_config, safety_settings=safety_settings)
        bot.send_message(
            "if item is not present in given data, assume its not trending", generation_config=generation_config, safety_settings=safety_settings)
...
with st.container(border=True):
    ...
    if 'prompt' in st.session_state and st.session_state['prompt'] != '':
        ...
        with st.spinner("Generating response..."):
            ...
            if condition:
                gen_prompt = img_gen_prompt(
                    st.session_state['prompt']) + " " + category
                print(gen_prompt)
                try:
                    _imgs = predict_image(
                        instance_dict={
                    ...
                    imgs = [io.BytesIO(base64.b64decode(
                        _img["bytesBase64Encoded"])) for _img in _imgs]
                ...
                try:
                    res = bot.send_message(
                        st.session_state['prompt'], generation_config=generation_config, safety_settings=safety_settings
                    )
                    res_text = res.text
                except:
                    res_text = 'Invalid prompt. Try again.'
