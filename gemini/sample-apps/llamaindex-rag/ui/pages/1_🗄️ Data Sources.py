...
def fetch_vector_search_indexes(qa_or_base: str):
    response = requests.post(f"{fastapi_url}/list_vector_search_indexes",
                             json={"qa_or_base": qa_or_base})
    if response.status_code == 200:
        return response.json()
    else:
        st.error("Failed to fetch Vector Search indexes. Please check the server connection.")
        return []

def fetch_vector_search_endpoints(qa_or_base: str):
    response = requests.post(f"{fastapi_url}/list_vector_search_endpoints",
                             json={"qa_or_base": qa_or_base})
    if response.status_code == 200:
        return response.json()
    else:
        st.error("Failed to fetch Vector Search endpoints. Please check the server connection.")
        return []
...
def fetch_firestore_databases():
    ...
    if response.status_code == 200:
        ...
        st.error("Failed to fetch Firestore collections. Please check the server connection.")
        return []

def fetch_firestore_collections(selected_database: str):
    response = requests.post(f"{fastapi_url}/list_firestore_collections", json={"firestore_db_name": selected_database})
    if response.status_code == 200:
        return response.json()
    else:
        st.error("Failed to fetch Firestore collections. Please check the server connection.")
...
def update_index(base_index_name: str,
    ...
    response = requests.post(f"{fastapi_url}/update_index",
                             json={"base_index_name": base_index_name,
...
if firestore_databases:
    selected_database = st.selectbox(
        "Select a Firestore Databases",
        index=0,
        options=firestore_databases + [None]
    )
...
if st.button("Submit Selection"):
    if base_selected_index and base_selected_endpoint:
        update_index(base_index_name=base_selected_index,
