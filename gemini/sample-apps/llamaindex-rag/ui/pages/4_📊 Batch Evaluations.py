...
def upload_to_gcs(source_file_name, destination_blob_name):
  try:
      ...
      cloud_logger.info(f"File {source_file_name} uploaded to {BUCKET_NAME}/{destination_blob_name}")
...
def call_eval_batch_api(payload):
  ...
  try:
      ...
      st.error("Request timed out. The batch operation might take longer than expected.")
...
st.set_page_config(layout="wide", page_title="RAG Batch Evaluation", page_icon=":robot_face:")

# Sidebar configurations
st.sidebar.markdown("#### 🛠️ Batch Evaluation Configurations")

st.sidebar.markdown("#### 🤖 LLM Model")
llm_name = st.sidebar.selectbox("Select a model:", ["gemini-1.5-flash", "gemini-1.5-pro", "gemini-1.0-pro"])

st.sidebar.markdown("#### 🤖 Eval LLM Model")
eval_model_name = st.sidebar.selectbox("Select Eval model:", ["gemini-1.5-flash", "gemini-1.5-pro", "gemini-1.0-pro"])
...
st.markdown("Please modify the configurations before uploading a CSV for batch evaluation. On upload, evaluation is automatically triggered using the LlamaIndex Agentic RAG.")
...
if uploaded_file is not None:
  try:
      ...
      if response:
        ...
        df['Question_Order'] = df['Question'].map({q: i for i, q in enumerate(ordered_questions)})
        ...
        melted_df = df.melt(id_vars=['Question'], 
                            value_vars=['Normalized Overall Score', 'Faithfulness', 'Answer Relevancy'],
                            var_name='Metric', value_name='Score')

        # Create the bar chart
        chart = alt.Chart(melted_df).mark_bar().encode(
            x=alt.X('Question:N', sort=ordered_questions, axis=alt.Axis(labelAngle=-45, labelLimit=200)),
            ...
            width=alt.Step(60),  # Adjust this value to change the width of the group of bars for each question
            height=400,
        ...
        st.info("Note: Overall Score has been normalized to a 0-1 range for consistency with other metrics.")
        ...
        for i, question in enumerate(ordered_questions):
            ...
            with st.expander(f"Question {i+1}"):
                ...
                st.markdown(f"**Ground Truth:** {response.get('ground_truth', [])[i]}")
