...
class ParentRetriever(BaseRetriever):
    ...
    def _retrieve(self, query_bundle: QueryBundle) -> List[NodeWithScore]:
        ...
        node_source_id_score_df = pd.DataFrame([{"source_id": n.node.relationships[NodeRelationship.SOURCE].node_id, 
                                                 "score": n.score} for n in initial_nodes])
        final_df = node_source_id_score_df.groupby('source_id')['score'].mean().reset_index()
