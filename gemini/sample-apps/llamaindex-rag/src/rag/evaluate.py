...
class LLMEvaluator(object):
    ...
    def __init__(self, system_prompt: str, user_prompt:str, eval_model_name: str, temperature: float):
        self.system_prompt = system_prompt
        self.user_prompt = user_prompt
        self.eval_model_name = eval_model_name

        if "gemini" in self.eval_model_name:
            self.eval_model = GenerativeModel(model_name=self.eval_model_name,
            ...
            self.eval_model = ClaudeVertexLLM(project_id="sysco-smarter-catalog",
                                              region="us-east5",
                                              model_name="claude-3-5-sonnet@20240620",
                                              max_tokens=1024,
                                              system_prompt=self.system_prompt)

    async def async_eval_answer(self,
        ...
        result = await generative_model.generate_content_async(self.user_prompt.format(question=question,
                                                                                        answer=answer,
    ...
    async def async_eval_question_answer_pair(self,
                                              retrieval_qa_func,
        ...
        score = await self.async_eval_answer(eval_model, question, answer, ground_truth, retrieved_context)
    ...
    async def async_eval_retrieval(self,
                                   retrieval_qa_func: Callable, 
                                   eval_df: pd.DataFrame) -> pd.DataFrame:
        results = await asyncio.gather(*[self.async_eval_question_answer_pair(retrieval_qa_func, 
                                                                self.eval_model, 
                                                                x["question"], 
                                                                x["ground_truth"]) for idx, x in eval_df[["question", "ground_truth"]].iterrows()]) 
...
def write_results_to_bq(pd_dataframe: pd.DataFrame, table_id: str = "eval_results.eval_results_table"):
