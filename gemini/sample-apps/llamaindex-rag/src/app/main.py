...
from src.rag.evaluate import write_results_to_bq, LLMEvaluator
...
logging.basicConfig(filename='eval.log', encoding='utf-8', level=logging.INFO)  # Set the desired logging level
...
ragas_metrics_dict = {"context_precision": context_precision,
...
async def list_vector_search_indexes(qa_or_base: dict, index_manager=Depends(get_index_manager)):
...
async def list_vector_search_endpoints(qa_or_base: dict, index_manager=Depends(get_index_manager)):
...
async def list_firestore_collections(db_name: dict, index_manager=Depends(get_index_manager)):
    """
    ...
    if db_name["firestore_db_name"]:
        db = firestore.Client(project=index_manager.project_id, database=db_name["firestore_db_name"])
    else:
        return []
    try:
        # Get all collections
        ...
        print(f"Permission denied. Make sure you have the necessary permissions to access Firestore in project {PROJECT_ID}")
        return []
    except Exception as e:
...
@app.post("/query_rag")
async def query_rag(rag_request: RAGRequest, index_manager=Depends(get_index_manager), prompts=Depends(get_prompts)):
...
@app.post("/eval_batch")
def eval_batch(eval_batch_request: EvalRequest, index_manager=Depends(get_index_manager), prompts=Depends(get_prompts)):
    query_engine = index_manager.get_query_engine(
                                        prompts=prompts,
    ...
    file_name = "/".join(eval_batch_request.input_eval_dataset_bucket_uri.split("/")[1:])
    ...
    llm_evaluator = LLMEvaluator(system_prompt=prompts.eval_prompt_wcontext_system,
    ...
    # create Langchain LLM and Embeddings
    vertextai_llm = ChatVertexAI(credentials=creds, model_name=eval_batch_request.eval_model_name)
    vertextai_embeddings = VertexAIEmbeddings(credentials=creds, model_name=eval_batch_request.embedding_model_name)
