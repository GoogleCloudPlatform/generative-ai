...
def soup_clean_html(
    ...
    allowlist_properties: tuple[str, ...] = (
        DEFAULT_CLEAN_HTML_ALLOWLIST_PROPERTIES
    ),
...
def should_destruct_table(table):
  ...
  if (
      max(len(tr.findAll('td', recursive=False)) for tr in table.findAll('tr'))
      <= 1
  ):
    return True
...
def fix_citations(md_text: str) -> str:
  ...
  while i_line < len(lines):
    ...
    for i_maybe_ref_line in range(
      i_line + 1,
      min(i_line + line_lookahead + 1, len(lines))
    ):
    ...
    replacement_line = lines[i_ref_line].replace(found_ref, found_ref.replace('\\', ''))
...
def split_lines_in_two(lines, split_start_idx, common_header_end_idx, common_footer_start_idx, max_chunk_size):
  assert split_start_idx > common_header_end_idx, f"{split_start_idx=} must be greater than {common_header_end_idx=}"
  assert split_start_idx < common_footer_start_idx, f"{split_start_idx=} must be less than {common_footer_start_idx=}"
...
def split_no_tables(input, max_chunk_size):
  ...
  def is_heading_or_blank(line):
    return IS_HEADING_RE.search(line) or IS_IMPLICIT_HEADING_RE.match(line) or not line.strip()

  content_start = 0
  while (content_start < len(lines) and is_heading_or_blank(lines[content_start])):
    content_start += 1

  empty_line_idx_list = [i for i, l in enumerate(lines[content_start:-1], content_start) if not l.strip()]

  if not empty_line_idx_list:
    token_length = len(input) // 5
    print(f"Only one paragraph left, cannot split further: {token_length=} > {max_chunk_size=}")
    return [{
        "text": input,
        "token_length": token_length,
    }]

  midpoint_start = find_nearest(empty_line_idx_list, (content_start + len(lines)) // 2) + 1

  return split_lines_in_two(lines, midpoint_start, content_start, len(lines), max_chunk_size)
...
def split_multiple_tables(input, max_chunk_size):
    ...
    mid_end_table_idx = find_nearest(end_table_idx_list, (first_start_table_end_idx + last_end_table_start_idx) / 2) + 1
    return split_lines_in_two(lines, mid_end_table_idx, first_start_table_end_idx, last_end_table_start_idx, max_chunk_size)
...
def split_single_table(input, max_chunk_size):
  ...
  try:
    table_rows_start_idx = next(
        (i + 1 for i, l in enumerate(lines[table_start_idx:], table_start_idx) if l.startswith('| - |')),
    )
  except ValueError as e:
    table_rows_start_idx = table_start_idx + 1

  table_subheader_start_idx_list = [
    i - 1 for i, l in enumerate(lines[table_rows_start_idx:], table_rows_start_idx) if l.startswith('| - |')
  ]
  ...
  if non_header_rows_count <= 1:
      token_length = len(input) // 5
      print(f"Only one row left in the table, cannot split further: {token_length=} > {max_chunk_size=}")
      return [{
          "text": input,
          "token_length": token_length,
      }]
  ...
  def split_table_in_half(splitpoint_start):
    ...
    return split_lines_in_two(lines, splitpoint_start, common_header_end_idx, common_footer_start_idx, max_chunk_size)

  if len(table_subheader_start_idx_list) > 1:
    # TODO: This assumes all lines are equal length, we could do better
    middle_subheader_start_idx = find_nearest(table_subheader_start_idx_list, (table_rows_start_idx + table_end_idx) / 2)
...
def split_more_to_chunks(
    chunk, target_chunk_size, max_chunk_size, target_heading_level,
    split_sections, split_implicit_headings,
  ):
...
def do_split_to_chunks(
    chunk_md, target_chunk_size, max_chunk_size, target_heading_level,
    split_sections, split_implicit_headings,
  ):
  ...
  def is_implicit_heading(block):
    return isinstance(block, marko.block.Paragraph) and len(block.children) == 1 and isinstance(block.children[0], marko.inline.StrongEmphasis)

  def remove_tail_empty_children(chunk):
    while len(chunk.children) > 0 and not markdown.render(chunk.children[-1]).strip():
      chunk.children.pop()
  ...
  def finish_chunk(allow_merge_with_following=True, new_chunk_with_headings=True):
    ...
    previous_unfinished_md = markdown.render(previous_unfinished_chunk) if previous_unfinished_chunk else ''
    ...
    if allow_merge_with_following and previous_plus_current_token_length <= target_chunk_size:
      ...
      if current_token_length > target_chunk_size or not allow_merge_with_following:
        # print("b2.b.1")
  ...
  for block in chunk.children:
    if isinstance(block, marko.block.Paragraph):
      ...
      if len(current_chunk.children) > 1 and not markdown.render(current_chunk.children[-1]).strip() and not markdown.render(block).strip():
...
def split_to_chunks(
    input, target_chunk_size = 1500, max_chunk_size=2500,
  ...
  if target_heading_level > 2:
    raise ValueError(f"'target_heading_level' must be <= 2, got {target_heading_level}")
