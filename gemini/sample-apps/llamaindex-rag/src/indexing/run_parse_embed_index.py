...
from common.utils import download_blob, download_bucket_with_transfer_manager, link_nodes
...
def main():
    ...
    download_bucket_with_transfer_manager(INPUT_BUCKET_NAME, prefix=BUCKET_PREFIX, destination_directory=local_data_path)

    # Parse documents using DocAI
    try:
        parsed_docs, raw_results = parser.batch_parse(blobs, chunk_size=CHUNK_SIZE, include_ancestor_headings=True)
        print(f"Number of documents parsed by Document AI: {len(parsed_docs)}")
        if parsed_docs:
            print(f"First parsed document text (first 100 chars): {parsed_docs[0].text[:100]}...")
        else:
    ...
    if QA_INDEX_NAME or QA_ENDPOINT_NAME:
        ...
        qa_index, qa_endpoint = get_or_create_existing_index(QA_INDEX_NAME,
                                                             QA_ENDPOINT_NAME,
                                                             APPROXIMATE_NEIGHBORS_COUNT)
        ...
        qa_extractor = QuestionsAnsweredExtractor(llm,
                                                  questions=5,
                                                  prompt_template=qa_extraction_prompt)
        async def extract_batch(li_docs):
            return await tqdm_asyncio.gather(*[qa_extractor._aextract_questions_from_node(doc) for doc in li_docs])
        ...
        async def parse_batch(metadata_list):
            return await asyncio.gather(*[program.acall(questions_list=x) for x in metadata_list], return_exceptions=True)
        ...
        for doc, questions in zip(li_docs, parsed_questions):
            if isinstance(questions, Exception):
                ...
                for q in questions.questions_list:
                    ...
                    q_doc.relationships[NodeRelationship.SOURCE] = RelatedNodeInfo(node_id=doc.doc_id)
                    q_docs.append(q_doc)
        docstore.add_documents(li_docs)
        storage_context = StorageContext.from_defaults(docstore=docstore, vector_store=qa_vector_store)
