...
class ProductDrafts:
    ...
    * Draft Organization: Arranges drafts based on their associated product titles.
    * Display: Renders drafts in an expandable format, including both image and text components.
    * Edit Integration: Provides buttons to trigger image editing and text regeneration
      for each draft.
    """
    ...
    def _display_draft_row(self, title_index: int):
        ...
        for j in range(st.session_state.num_drafts):
            ...
            with text_col:
                ...
                if st.button(
                    "Regenerate",
                    key=f"""edit text{st.session_state.chosen_titles[title_index]}
    ...
    def _handle_image_edit(self, title_index: int, draft_index: int):
        ...
        if st.button("Edit Image", key=button_key, type="primary"):
            # Store information in session state to track which image is being edited
