...
class VectorSearchUtils:
    ...
    def get_index_endpoint(self):
        ...
        page_result = \
          self.index_endpoint_client.list_index_endpoints(request=request)
        index_endpoints = [
            response.name
        ...
        if len(index_endpoints) == 0:
            ...
            index_endpoint = \
              self.index_endpoint_client.get_index_endpoint(request=request)
    ...
    def create_index(
        ...
        if index:
            logger.info("Index %s already exists with id %s", \
              self.index_name, index.name)
        else:
            logger.info("Index %s does not exists. Creating index ...", \
              self.index_name)
            ...
            if index_update_method == "streaming":
                ...
                index_update_method = \
                  aipv1.Index.IndexUpdateMethod.BATCH_UPDATE
            ...
            if index_algorithm == "treeah":
                algorithm_config = struct_pb2.Struct(
                    fields={
                        "treeAhConfig": struct_pb2.Value(\
                          struct_value=tree_ah_config)
                    }
                )
            else:
                algorithm_config = struct_pb2.Struct(
            ...
            config = struct_pb2.Struct(
                fields={
                    ...
                    "algorithmConfig": struct_pb2.Value(\
                      struct_value=algorithm_config),
                    "shardSize": struct_pb2.Value(\
                      string_value="SHARD_SIZE_SMALL"),
                }
            )
            ...
            index = r.result()
            logger.info("Index %s created with resource name as %s", \
              self.index_name, index.name)
    ...
    def deploy_index(
        ...
        try:
            ...
            if index_endpoint:
                logger.info("Index endpoint %s already exists with resource \
                  name as %s and endpoint domain name as %s",\
                    self.index_endpoint_name, index_endpoint.name, \
                ...
                logger.info("Index endpoint %s \
                  created with resource name as %s and \
                    endpoint domain name as %s", \
                      self.index_endpoint_name, index_endpoint.name,\
                        index_endpoint.public_endpoint_domain_name)
        except Exception as e:  # pylint:disable=C0103
            logger.error("Failed to create index endpoint %s\n %s", \
              self.index_endpoint_name, e)
            raise e
        ...
        try:
            ...
            for d_index in index_endpoint.deployed_indexes:
                if d_index.index == index.name:
                    logger.info("Skipping deploying Index. \
            ...
            deployed_index_id = \
              f"{self.index_name.replace('-', '_')}_{timestamp}"
            deploy_index = {
            ...
            logger.info("Deployed index %s to endpoint %s", \
                    self.index_name, self.index_endpoint_name)

        except Exception as e:  # pylint:disable=C0103
            logger.error("Failed to deploy index %s to the index endpoint %s",\
              self.index_name, self.index_endpoint_name)
            raise e
    ...
    def delete_index(self):
        ...
        if index:
            # Delete index
            index_id = index.name
            logger.info("Deleting Index %s with id %s", \
    ...
    def delete_index_endpoint(self):
        ...
        # Create Index Endpoint if does not exists
        if index_endpoint:
            logger.info("Index endpoint %s exists \
            ...
            for d_index in index_endpoint.deployed_indexes:
                logger.info("Undeploying index %s from Index endpoint %s", \
                      d_index.id, self.index_endpoint_name)
            ...
            raise Exception(f"Index endpoint {self.index_endpoint_name}\
               does not exists.")
...
def deploy_index_endpoint(logger, project_id, me_region, me_index_name):
    ...
    if list_endpoints:
        ...
        logger.info(\
          "Creating new endpoint as none existed from previous run.")
        index_endpoint = mengine.deploy_index()
        if index_endpoint:
            logger.info("Index endpoint resource name: %s",index_endpoint.name)
            logger.info("Index endpoint public domain name: %s", \
              index_endpoint.public_endpoint_domain_name)
