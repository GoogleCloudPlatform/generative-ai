        ...
        try:
            ...
            async with client.aio.live.connect(
                ...
                for task in done:
                    try:
                        result = task.result()
                        if result == "reconnect" or (
                            result == "ended" and session_active
                        ):
...
def handle_video(data):
    ...
    if session_id and frame:
        get_session_state(session_id)["last_seen_frame"] = (
    ...
    if session_id in bridges and frame:
        try:
            ...
            logging.exception(f"Frame error: {e}")
...
def handle_uploaded_images(data):
    ...
    if session_id in bridges and session_id in live_sessions:
        try:
            ...
            logging.exception(f"Image upload error: {e}")
...
def handle_message_with_images(data):
    ...
    try:
        ...
        logging.exception(f"Message with images error: {e}")
        return {"status": "error", "message": str(e)}
    ...
    if session_id in bridges and call_id:
        response = [
            types.FunctionResponse(
                ...
                response={
                    "status": "completed",
                    "result": f"Generated {gen_type} {gen_id}",
                },
            )
        ]
...
def handle_genmedia_chat():
    try:
        ...
        if isinstance(tool_decision, dict):
            tool_name = (
                tool_name
                or tool_decision.get("function_name")
        ...
        if tool_name in ["veo", "generate_video"]:
            return handle_video_generation(args, session_id)
        if tool_name == "extract_frame":
        ...
        if tool_name == "view_generated_image":
            ...
            return jsonify(
                {"response": f"Image {image_id} loaded", "image_id": image_id}
            )
...
def handle_video_generation(args, session_id):
    ...
    try:
        ...
        if reference_image and reference_image.exists():
            ...
            operation = video_client.models.generate_videos(
                model=VEO_MODEL_ID, prompt=prompt, config=video_config
            )
        ...
        result = (
            operation.result if hasattr(operation, "result") else operation.response
        )
        ...
        logging.exception(f"Video generation error: {e}")
        traceback.print_exc()
        return jsonify({"error": str(e)}), 500
...
def handle_extract_frame(args, session_id):
    ...
    try:
        ...
        logging.exception(f"Frame extraction error: {e}")
        traceback.print_exc()
        return jsonify({"error": str(e)}), 500
...
def handle_combine_videos(args, session_id):
    ...
    try:
        ...
        logging.exception(f"Video combination error: {e}")
        traceback.print_exc()
        return jsonify({"error": str(e)}), 500
...
def clear_all():
    ...
    session_handles.pop(session_id, None)
