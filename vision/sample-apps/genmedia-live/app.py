...
def get_next_id_for_type(file_type, session_id):
    existing = [e["id"] for e in file_history if e["type"] == file_type and e.get("session_id") == session_id]
    return 1 if not existing else max(existing) + 1


def get_last_id_for_type(file_type, session_id):
    session_files = [f for f in file_history if f.get('session_id') == session_id and f.get('type') == file_type]
...
def get_active_image_client():
    ...
    creds_data = session_credentials['oauth']
    creds = Credentials(token=creds_data['access_token'])
    return genai.Client(vertexai=True, project=creds_data['project_id'], location="global", credentials=creds)


def get_live_system_prompt(is_resumed=False):
    global user_name, custom_system_instructions
...
async def run_live_session(session_id, sid):
    ...
    while reconnect_count < max_reconnects:
        ...
        try:
                ...
                types.Tool(function_declarations=[
                    types.FunctionDeclaration(
                        ...
                        parameters={
                            ...
                            "properties": {
                                ...
                                "refers_to_camera": {"type": "boolean", "description": "Use camera/uploaded image as reference"},
                                "refers_to_last": {"type": "boolean", "description": "Use last generated image as reference"},
                                "image_id": {"type": "integer", "description": "Specific image ID to reference"},
                            },
                            "required": ["prompt"]
                        }
                    ),
                    types.FunctionDeclaration(
                        ...
                        parameters={
                            ...
                            "properties": {
                                ...
                                "duration": {"type": "integer", "description": "Duration in seconds (4, 6, or 8)"},
                                "refers_to_last": {"type": "boolean", "description": "Animate the last generated image"},
                                "image_id": {"type": "integer", "description": "Specific image ID to animate"},
                            },
                            "required": ["prompt"]
                        }
                    ),
                    types.FunctionDeclaration(
                        ...
                        parameters={
                            "type": "object",
                            "properties": {
                                "video_id": {"type": "integer", "description": "Video ID (defaults to last video)"},
                    ...
                    types.FunctionDeclaration(
                        ...
                        parameters={
                            "type": "object",
                            "properties": {
                                "video_ids": {"type": "array", "items": {"type": "integer"}, "description": "Video IDs to combine"},
                            },
                            "required": ["video_ids"]
                        }
                    ),
                    types.FunctionDeclaration(
                        ...
                        parameters={
                            "type": "object",
                            "properties": {
                                "image_id": {"type": "integer", "description": "Image ID to view"},
                            },
                            "required": ["image_id"]
                        }
                    ),
                            ...
                            if item['type'] == 'audio':
                                ...
                                for img_data in item.get('images', []):
                                    if isinstance(img_data, dict):
                                        blob = types.Blob(mime_type=img_data.get('mime_type', 'image/jpeg'), data=img_data['data'])
                                        for _ in range(5):
                                            await session.send_realtime_input(video=blob)
                                            await asyncio.sleep(0.1)
                                await session.send_client_content(
                                    turns=types.Content(role="user", parts=[types.Part(text=item.get('text', ''))]),
                                    turn_complete=True
                                )
                                ...
                                img_data = item['data']
                                blob = types.Blob(mime_type=img_data.get('mime_type', 'image/png'), data=img_data['data'])
                                await session.send_client_content(
                                    turns=types.Content(role="user", parts=[
                                        types.Part(inline_data=blob),
                                        types.Part(text=f"[Generated image {item.get('id', '?')}]")
                                    ]),
                                    turn_complete=False
                                )
                            ...
                            async for response in session.receive():
                                ...
                                if response.tool_call:
                                    for fc in response.tool_call.function_calls:
                                        socketio.emit('voice_generation_request', {
                                            'text': "Generating...",
            ...
            if session_active and (should_reconnect or has_handle):
                reconnect_count += 1
                logging.info(f"Reconnecting session {session_id} (attempt {reconnect_count}, has_handle={has_handle})")
    ...
    if session_id in live_sessions:
        ...
        socketio.emit('session_ended_reconnect', {
            ...
            'can_resume': session_id in session_handles
        }, room=final_sid)
    starting_sessions.discard(session_id)
            ...
            for img_data_url in images:
                if isinstance(img_data_url, str) and ',' in img_data_url:
                    ...
                    if PIL_AVAILABLE:
                        ...
                        processed_images.append({'mime_type': 'image/jpeg', 'data': buffer.getvalue()})
                    else:
...
def handle_genmedia_chat():
    try:
        ...
        if isinstance(tool_decision, dict):
            tool_name = tool_name or tool_decision.get("function_name") or tool_decision.get("tool")

        args = {}
        if data.get("function_args") and isinstance(data.get("function_args"), dict):
            args = data.get("function_args")
        elif tool_decision.get("function_args") and isinstance(tool_decision.get("function_args"), dict):
        ...
        if tool_name in ["nano_banana", "generate_image"]:
            ...
            return handle_extract_frame({'video_id': video_id, 'timestamp': args.get('timestamp', 0)}, session_id)
        elif tool_name == "combine_videos":
            video_ids = args.get("video_ids", [])
            if len(video_ids) < 2:
                session_videos = [f for f in file_history if f.get('session_id') == session_id and f.get('type') == 'video']
...
def handle_image_generation(args, session_id):
    ...
    if specific_image_id:
        session_images = [f for f in file_history if f.get('session_id') == session_id and f.get('type') == 'image']
    ...
    try:
        ...
        for ref_img in ref_images[:4]:
            if isinstance(ref_img, str) and ref_img.startswith('data:'):
                try:
                    ...
                    content_parts.append({"inline_data": {"mime_type": "image/png", "data": b}})
                except Exception:
                    pass
        ...
        if client is None:
            return jsonify({"error": "Authentication required. Please enter your Project ID and access token."}), 401
        ...
        file_history.append({"id": next_id, "type": "image", "path": str(image_path), "session_id": session_id})

        return jsonify({
            "response": f"Generated image {next_id}",
            "file": {"id": next_id, "type": "image", "url": f"/outputs/images/image_{next_id}.png"}
        })
    except Exception as e:
...
def handle_video_generation(args, session_id):
    ...
    try:
        ...
        creds_data = session_credentials['oauth']
        creds = Credentials(token=creds_data['access_token'])
        video_client = genai.Client(vertexai=True, project=creds_data['project_id'], location="global", credentials=creds)
        ...
        if reference_image and reference_image.exists():
            operation = video_client.models.generate_videos(
                model=VEO_MODEL_ID,
                prompt=prompt,
                image=types.Image(image_bytes=reference_image.read_bytes(), mime_type="image/png"),
                config=video_config
            )
        else:
            operation = video_client.models.generate_videos(
        ...
        file_history.append({"id": next_id, "type": "video", "path": str(video_path), "session_id": session_id})

        return jsonify({
            "response": f"Generated video {next_id}",
            "file": {"id": next_id, "type": "video", "url": f"/outputs/videos/video_{next_id}.mp4"}
        })
...
def handle_extract_frame(args, session_id):
    ...
    try:
        ...
        cmd = ["ffmpeg", "-y", "-ss", str(timestamp), "-i", str(video_path), "-vframes", "1", "-f", "image2", "-vcodec", "png", str(frame_path)]
        ...
        file_history.append({"id": next_id, "type": "image", "path": str(frame_path), "session_id": session_id})
...
def handle_combine_videos(args, session_id):
    ...
    try:
        ...
        if result.returncode != 0:
            cmd = ["ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", concat_file, "-c:v", "libx264", "-c:a", "aac", "-preset", "fast", str(output_path)]
        ...
        file_history.append({"id": next_id, "type": "video", "path": str(output_path), "session_id": session_id})
...
def auth_status():
    if 'oauth' in session_credentials:
        return jsonify({
            "authenticated": True,
            "project": session_credentials['oauth']['project_id']
        })
    return jsonify({
        "authenticated": False,
